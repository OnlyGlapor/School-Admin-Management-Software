...
const CreateStudentModal = ({openCreateStudentModal, handleCreateStudentModal}) => {
  ...
  return(
    <CustomDialog isOpen={openCreateStudentModal} onRequestClose={() => setOpenCreateStudentModal(false)}>
      <>
      {createProfile === 'Step 1' && <CreateProfilePrompt handleCreateStudentModal={handleCreateStudentModal} handleNextButton={handleNextButton} />}
      {createProfile === 'Step 2' && <CreateProfile studentStatus={studentStatus} grades={grades} handleCreateStudentModal={handleCreateStudentModal} handleNextButton={handleNextButton} handleBackButton={handleBackButton} handleOpenCancellationModal={handleOpenCancellationModal} />}
      {createProfile === 'Step 3' && <DocumentSubmissionStep handleCreateStudentModal={handleCreateStudentModal} handleBackButton={handleBackButton} />}
...
const CreateProfilePrompt = ({handleNextButton, handleCreateStudentModal}) => {
  return(
    <div className="w-3/5 h-3/4 rounded-lg bg-white p-5 relative flex flex-col justify-center overflow-hidden">
      <div className="flex justify-center z-50">
        <button onClick={handleCreateStudentModal} className="w-9 h-9 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center text-lg cursor-pointer absolute right-2 top-2">
         <IoClose />
        </button>
        <div className="flex flex-col items-start w-2/3 pl-5">
          ...
          <div className="mt-5 shadow-lg">
            <p className="text-xl font-bold text-white">Create Student Profile</p>
      ...
      <div className="absolute left-0 w-1/2">
        <img src="https://scontent.fmlw1-1.fna.fbcdn.net/v/t39.30808-6/403751789_666524762269898_5639827366292604568_n.jpg?_nc_cat=101&ccb=1-7&_nc_sid=efb6e6&_nc_eui2=AeGVp_GEBoWbqSv_rqC3Npc2g-XZwF__Wf-D5dnAX_9Z_ycs7q37t8teYO1Pmvd_xgGt4U0Xi2QUwRUiRpIEM7ER&_nc_ohc=cDa-GtBbl6YAX_d1YE0&_nc_oc=AQniCgXBcoAl8epHwvIpGCfnguxvxs_KJglUUFT57T5-ShMn2HubGS1lGCtgN94DBsM&_nc_zt=23&_nc_ht=scontent.fmlw1-1.fna&oh=00_AfC4cYVVM5UqgfxeAJUA8nYRCktpZamqDp84Xbj3NhAXFA&oe=6570B478" />
      </div>
...
const CreateProfile = ({studentStatus, grades, handleCreateStudentModal, handleNextButton, handleBackButton, handleOpenCancellationModal}) => {
  return(
        ...
        <div>
          <div className="flex justify-start mt-2 px-10 py-5">
            <div>
              ...
              <DropDown title="User Type" options={userTypes} />
...
const DropDown = ({title, options}) => {
  ...
  return(
    <div className="flex items-center my-4">
      <p className="text-sm font-semibold w-32">{title}:</p>
      <select className="bg-white border w-40 ml-2 rounded-md p-1 text-sm">
